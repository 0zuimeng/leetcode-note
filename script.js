...
// 修改 organizeProblemsByRounds 以支持专栏过滤
function organizeProblemsByRounds() {
    problemsData = {
        round1: { name: "第一轮", description: "简单 50%+", categories: {} },
        round2: { name: "第二轮", description: "中等 50%+", categories: {} },
        round3: { name: "第三轮", description: "算法理论/进阶", categories: {} },
        round4: { name: "第四轮", description: "困难/挑战", categories: {} }
    };

    const round3AlgoCats = ['树', '图与回溯算法', '贪心', '动态规划'];
    const round3SqlCats = ['聚合函数', '分组循环', '高级连接', '子查询'];

    // 过滤当前专栏题目
    const filtered = allProblems.filter(p => (p.type || 'algo') === currentColumn);

    filtered.forEach(problem => {
        const { difficulty, passRate, category, id } = problem;
        let targetRound = 'round4';

        if (currentColumn === 'algo' && round3AlgoCats.includes(category)) targetRound = 'round3';
        else if (currentColumn === 'sql' && round3SqlCats.includes(category)) targetRound = 'round3';
        else if (difficulty === '简单' && passRate >= 50) targetRound = 'round1';
        else if (difficulty === '中等' && passRate >= 50) targetRound = 'round2';

        if (!problemsData[targetRound].categories[category]) {
            problemsData[targetRound].categories[category] = [];
        }
        problemsData[targetRound].categories[category].push(id);
    });

    Object.keys(problemsData).forEach(rk => {
        const obj = problemsData[rk].categories;
        problemsData[rk].categories = Object.keys(obj).map(name => ({ name, problems: obj[name] }));
    });
}
...
function openCheckInModal(roundKey, problemNum, element) {
    pendingCheckIn = { roundKey, problemNum, element };
    document.getElementById('checkInTitle').textContent = `题目 ${problemNum} 打卡`;

    const now = new Date();
    now.setMinutes(now.getMinutes() - now.getTimezoneOffset());
    document.getElementById('checkInTimeInput').value = now.toISOString().slice(0, 16);
...
document.getElementById('confirmCheckInBtn').onclick = function () {
    ...
    const { roundKey, problemNum } = pendingCheckIn;
    const timeVal = document.getElementById('checkInTimeInput').value;

    if (!userProgress[roundKey]) userProgress[roundKey] = {};
    userProgress[roundKey][problemNum] = {
        solvedAt: timeVal ? new Date(timeVal).toISOString() : new Date().toISOString(),
        round: roundKey
    };
...
// 最后，将原来的 toggleProblem 中“打卡”的部分替换为：
// else { openCheckInModal(roundKey, problemNum, element); }
// 替换原有的 toggleProblem 函数
function toggleProblem(roundKey, problemNum, element) {
    if (userProgress[roundKey] && userProgress[roundKey][problemNum]) {
        if (confirm(`确定要取消题目 ${problemNum} 的打卡记录吗？`)) {
        ...
        openCheckInModal(roundKey, problemNum, element);
    }
}
